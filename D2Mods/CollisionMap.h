...
BOOL CCollisionMap::BuildMapData(DWORD AreaIds[], int nSize)
{
	...
	Level* pBestLevel = GetLevel(D2CLIENT_GetPlayerUnit()->pAct, AreaIds[0]);
	DWORD dwXSize = 0;
	DWORD dwYSize = 0;
	...
	for (int n = 0; n < nSize; n++) {
		...
		Level* pLevel = GetLevel(D2CLIENT_GetPlayerUnit()->pAct, AreaIds[n]);
...
int CCollisionMap::GetLevelExits(LPLevelExit* lpLevel)
{
	...
	for (Room2* pRoom = GetLevel(D2CLIENT_GetPlayerUnit()->pAct, dwLevelId)->pRoom2First; pRoom; pRoom = pRoom->pRoom2Next)
	{
