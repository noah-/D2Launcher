...
enum {
	...
	PATH_REACHED
};    // Path OK, destination reached(Path finding completed successfully)
	...
	for (int x = 0; x < m_nCX; x++)
	{
		for (int y = 0; y < m_nCY; y++)
		{
			if ((m_ppTable[x][y] % 2) == 0)
				m_ppTable[x][y] = (short)GetDistanceSquared(x, y, m_ptEnd.x, m_ptEnd.y);
			else
...
{
	if (GetDistanceSquared(m_ptEnd.x, m_ptEnd.y, pos.x, pos.y) <= TP_RANGE)
	{
		pos = m_ptEnd;
	...
	for (p.x = pos.x - TP_RANGE; p.x <= pos.x + TP_RANGE; p.x++)
	{
		for (p.y = pos.y - TP_RANGE; p.y <= pos.y + TP_RANGE; p.y++)
		{
			...
			if (m_ppTable[p.x][p.y] < value && GetDistanceSquared(p.x, p.y, pos.x, pos.y) <= TP_RANGE)
			{
...
int CTeleportPath::GetRedundancy(const LPPOINT lpPath, DWORD dwMaxCount, const POINT& pos)
{
	...
	{
		if (GetDistanceSquared(lpPath[i].x, lpPath[i].y, pos.x, pos.y) <= TP_RANGE / 2)
			return i;
	}
